...
参考对象
- 集成平台玩家：GitHub Copilot（PR 功能强，但不服务 GitLab 用户）。
- AI 原生审查者：CodeRabbit（支持 GitLab，摘要/逐行/对话/图表，强调安全与临时处理）。
- 静态分析：SonarQube / Codacy（规则/质量门禁与指标成熟，逐步引入 AI）。

MoonLens 差异点
- GitLab 原生优先，Webhook→队列→AI 服务→MR 评论完整闭环；
- Provider 可插拔与私有化：支持 OpenAI/Anthropic/Azure/代理，组织/项目级路由与限额；
- 零持久化与最小权限：diff 非持久；密钥加密存储；日志脱敏；
- 与 SAST 互补：计划聚合 Sonar/Codacy 结果形成统一质量视图与门禁阈值。

MVP 与差距（可度量）
- 已具备：MR 摘要、逐行建议、多 Provider、Webhook/队列、评论回写；
- 待补齐：对话式审查（Q2 2025）、外部静态分析聚合（Q2/3 2025）、质量门禁（可选）、图表与趋势；
- KPI：建议采纳率、误报率、平均审查时延、回写成功率、活跃项目数。
